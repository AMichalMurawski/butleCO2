...
export const validateClientSchema = Yup.object().test(
  ...
  function (value: any) {
    ...
    const hasPhone = typeof phone === 'string' && /^(\d{2}-\d{3}-\d{2}-\d{2}|\d{3}-\d{3}-\d{3})$/.test(phone);
    const hasEmail = typeof email === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
    const hasValidDeliveryTime = Array.isArray(deliveryTime) && deliveryTime.some(d => d.enabled);
    const isValidMessage = typeof message === 'undefined' || (typeof message === 'string' && message.length <= 150);
