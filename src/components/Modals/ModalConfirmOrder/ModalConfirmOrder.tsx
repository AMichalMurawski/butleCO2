...
import { ConfirmOrderWraper, InvoiceWraper, SegmentWraper, Subtitle, TextLine, TextTitle, TextValue, Title, TitleWraper } from "./ModalConfirmOrder.styled";

const ModalConfirmOrder: React.FC = () => {
    const { order } = useOrder();

    return <ConfirmOrderWraper>
        <TitleWraper>
        ...
        <InvoiceWraper>
            <SegmentWraper>
                ...
                <TextLine>
                    ...
                    <TextValue>{order.client.address.street} {order.client.address.number}{order.client.address.local ? ` lok. ${order.client.address.local}` : null}, {order.client.address.postalCode} {order.client.address.city}</TextValue>
                </TextLine>
                <TextLine>
            ...
            {order.client.invoice &&
                (<SegmentWraper>
                    ...
                    <TextLine>
                        ...
                        <TextValue>{order.company.address.street} {order.company.address.number}{order.company.address.local ? ` lok. ${order.company.address.local}` : null}, {order.company.address.postalCode} {order.company.address.city}</TextValue>
                    </TextLine>
                    <TextLine>
            ...
            <SegmentWraper>
                ...
                {order.products.map(product => {
                    ...
                    return (
                        <TextLine>
                            <TextTitle>{product.type} - {product.weight ? (`${product.weight} kg`) : (`${product.litr} l`)}:</TextTitle>
                            <TextValue>{product.unitPrice}PLN x {product.amount}szt. = {product.price}PLN</TextValue>
                        </TextLine>
                    )
                }
                )}
